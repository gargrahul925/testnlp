...
module.exports = function () {
    this.registerUser = function (db, userID, userName, posterImage,platform, callback) {
        ...
        test.count(function (err, count) {
            if (count == 0) {
                ...
                callback({ "status": -1, "Error": 200, "msg": "user Id is already registered" }, true)
            }
        });
    };
        ...
        db.collection('users').updateOne({
            ...
            function (err, results) {
                if (results != null) {
                    ...
                    callback({ "status": -1, "Error": 201, "msg": "token not generated" }, true);
                }
            });
    };
    ...
    this.getUserInfo = function (db, userID, callback) {
        ...
        cursor.nextObject(function (err, item) {
            if (item != null) {
				//console.log((new Date()).getTime());
                callback({ "name": item.fbName, "image": item.fbImage }, false);
            } else {

                callback({ "status": -1, "Error": 202, "msg": "userId not found" }, true);
            }
        });
    };
                ...
                db.collection('userpolls').insertOne({
                    ...
                    cursor.count(function (err, count) {
                        if (count == 1) {
                            ...
                            callback({ "status": -1, "Error": 203, "msg": "poll not inserted correctly in database" }, true);
                        }
                    });
                });
            } else {

                callback({ "status": -1, "Error": 204, "msg": "incomplete user information" }, true);
            }
        });
            ...
            function (err, results) {
                ...
                if (results != "") {
                    if (results["result"]["n"] == 1)
                        ...
                        callback({ "status": -1, "Error": 217 }, true);
                }
                else {
                    callback({ "status": -1, "Error": 218 }, true);
                }
    ...
    this.retrievePoll = function (db, poll_id, user_id, callback) {
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                ...
                callback({ "status": -1, "Error": 205, "msg": "No poll found with passed poll_id" }, true);
            }
        });
    };
    ...
    this.answerPoll = function (db, poll_id, user_id, answer_number, callback) {
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                cursor.nextObject(function (err, doc) {
                    if (doc.is_active == 0) {
                        callback({ "status": -1, "Error": 206, "msg": "referenced poll is inactive" }, true);
                        return;
                    }
                    assert.equal(err, null);
                        ...
                        function (err, results) {
                            ...
                            answercursor.count(function (err, count) {
                                if (count == 1) {
                                    ...
                                    callback({ "status": -1, "Error": 207, "msg": "No answer found with passed poll_id" }, true);
                                }
                            });
                ...
                callback({ "status": -1, "Error": 208, "msg": "No poll found with passed poll_id" }, true);
            }
        });
    };
    ...
    this.addParticipant = function (db, poll_id, participant_ids, callback) {
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                ...
                callback({ "status": -1, "Error": 209, "msg": "invalid poll id" }, true);
            }
        });
    };
    ...
    this.fetchCommentsResults = function (db, poll_id, callback) {
        ...
        answercursor.count(function (err, count) {
            if (count == 1) {
                ...
                callback({ "status": -1, "Error": 210, "msg": "invalid Poll Id" }, true);
            }
        });
    };
    ...
    this.addComment = function (db, poll_id, user_id, comment, callback) {
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                        ...
                        function (err, results) {
                            ...
                            answercursor.count(function (err, count) {
                                if (count == 1) {
                                    ...
                                    callback({ "status": -1, "Error": 211, "msg": "poll Id not found with required parameters" }, true);
                                }
                            });
                        });
                ...
                callback({ "status": -1, "Error": 212, "msg": "invalid poll Id" }, true);
            }
        });
    };
...
console.log(launch_t);
		var cursor ;
		var findObj = {participants: user_id,
				prev_poll: "",
				};
				if(launch_t !=null && parseInt(launch_t) == launch_t){
					findObj.launch_time = {$lt:launch_t} 
				}
        ...
        cursor.count(function (err, count) {
            if (count != 0) {
                ...
                callback({ "status": -1, "Error": 213, "msg": "No polls found with current user id" }, true);
            }
        });
    };
            ...
            cursor.count(function (err, count) {
                if (count >= 1) {
                    ...
                    cursor.each(function (err, result) {
                        ...
                        i++;
                        if (i == count) {
                            callback({ "status": 1, "data":  JSON.stringify(data_to_return)  }, false);
                        }
                    });
                } else {
                    callback({ "status": -1, "Error": 214, "msg": "No Polls" }, true)
                }
            });
        });
    }
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                cursor.nextObject(function (err, item) {
                    ...
                    if (!allowed) {
                        callback({ "status": -1, "msg": "Duplicate Option" }, true)
                    } else {
                        ...
                        db.collection('userpolls').updateOne({
                            ...
                            function (err, results) {
                                ...
                                notifyForPoll(db, poll_id, user_id, data, null);
                                callback({ "status": 1, "data": JSON.stringify(item.options_data) }, false);
                            }
                        );
                    }
                });
            } else {
                callback({ "status": -1, "Error": 215, "msg": "Incorrect Poll Id" }, true)
            }
        });
    }
    ...
    this.notifyForPoll = function (db, poll_id, user_id, data, callback) {
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                ...
                callback({ "status": -1, "Error": 216 }, true);
            }
        });
    }
    ...
    this.sendGcmMessageSystem = function (db, tokens, data) {
        var message = new gcm.Message({priority: 'high'});
        message.addData('data', data);
        ...
        cursor.count(function (err, count) {
            if (count == 1) {
                cursor.nextObject(function (err, item) 
				{
					...
					else if(data.type == 2) //has added an option
					{
						message.addNotification('body', data.user_name + " has added an option \""+data.option+"\" to your question \""+item.question_data.data+"\"");
					}
