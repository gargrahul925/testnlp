...
if (process.argv.length != 4) {
    ...
    MongoClient.connect(mongoURL, function(err, db) {
        ...
        server.register(Basic, function(err) {
                            ...
                            http.get(options, function(res) {
                                if (res.statusCode == 200) {
                                    ...
                                    reply({
                                        "status": -1,
                                        "error": "122",
                                        "msg": "response returned " + res.statusCode
                                    });
                                }
                            }).on('error', function(e) {
                                reply({
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        ...
                        var platform = request.payload.platform ? parseInt(request.payload.platform) : 0;

                        if (fb_user_id != "" && user_name != "") {
                            registerUser(db, fb_user_id, user_name, profile_pic, platform, function(returnVal, err) {
                                if (!err) {
                                    reply({
                                        "status": 1,
                                        "data": returnVal
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        var user_id = request.payload.user_id ? request.payload.user_id : "";
                        var token = request.payload.token ? request.payload.token : "";
                        var platform = request.payload.platform ? request.payload.platform : 0;
						

                        if (token != "" && user_id != "") {
                            registerToken(db, user_id, token,platform, function(returnVal, err) {
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        var user_id = request.payload.user_id ? request.payload.user_id : "";
                        var poll_id = request.payload.poll_id ? request.payload.poll_id : null;
                        var launch_time = request.payload.launch_time ? request.payload.launch_time : null;
                        launch_time = parseInt(launch_time);

                        if (user_id != "") {
                            retrievePollsByUserID(db, user_id, poll_id, launch_time, function(returnVal, err) {
                                if (!err) {
                                    reply({
                                        "status": 1,
                                        "data": returnVal
	    ...
	    server.route({
		method:'GET',
		path:'/owlie/questioncontext',
		config: {
	 	    auth: 'simple',
		    handler: function(request, reply){

			var url = "api.projectoxford.ai";
			var luis_id = "f9b1d831-c63c-4846-841b-35ba1692ca46";
			var sub_key = "c1c23b1a39194d3a8869789edb99f2b3";

			var cacheData = cache.get(request.query.q);
			if (cacheData != undefined) {
				...
				var options = {
					host: url,
					port: 443,
					path: '/luis/v1/application?id=' +luis_id + '&subscription-key=' + sub_key + '&q=',
					method: 'GET'
				};
				var question = request.query.q ? request.query.q:"";
				options.path += encodeURIComponent(question);
				https.request(options, function(res) {
					res.setEncoding('utf8');
					res.on('data', function (chunk) {
						var data = JSON.parse(chunk);
						var pre, at, post;
						var favouredIntent = data.intents[0].intent;
						var topScore = data.intents[0].score;
						if(topScore > 0.3 && favouredIntent != "None")
						{
							switch (favouredIntent) {
								case "ShowPlacePredictionBookCab":
								{
									pre = "ShowPlacePrediction";
									at = "";
									post = "BookCab";
									...
									cache.set(request.query.q, context_data);
								}
								break;
								case "OutStationPredictionWeatherChannel":
                                                                {
                                                                        pre = "ShowPlacePrediction";
                                                                        at = "";
                                                                        post = "WeatherChannel";
