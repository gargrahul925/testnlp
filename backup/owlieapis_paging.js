        ...
        server.register(Basic, function(err) {
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        ...
                        if (who_id != "" && back_image != null && question_data != "" && options_data != "") {
                            createPoll(db, poll_id, who_id, back_image, launch_time, JSON.parse(question_data.replace("\\\"", "\"")), JSON.parse(options_data.replace("\\\"", "\"")), timer_value, prev_poll,story_caption, isMultiSelect ,function(returnVal) {
                                reply(returnVal);
                            });
                        } else {
                            reply('{"Error":102}')
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        ...
                        if (poll_id != "" && user_id!="" ) {
                            closePoll(db, poll_id,user_id,caption, function(returnVal) {
                                reply(returnVal);
                            });
                        } else {
                            reply('{"Error":102}')
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        var user_id = request.payload.user_id ? request.payload.user_id : "";
                        var page_num = request.payload.page_num ? request.payload.page_num : 0;
                        if (user_id != "") {
                            retrievePollsByUserID(db, user_id, page_num, function(returnVal) {
                                reply(returnVal);
                            });
                        } else {
                            reply('{"Error":102}')
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
                        ...
                        if (poll_id != "" && user_id != "") {
                            pokeForPoll(db, poll_id, user_id,story_caption, function(returnVal, index) {
                                reply(returnVal);
                            });
                        } else {
                            reply('{"Error":102}')
			...
			var user_name = request.payload.user_name ? request.payload.user_name :"";
                        ...
                        if (poll_id != "" && option_data != "" && user_id!="" && user_name != "") {
                            addOption(db, poll_id,user_id,user_name, option_data, function(returnVal, index) {
                                reply(returnVal);
                            });
                        } else { 
            ...
            server.route({
                ...
                config: {
                    ...
                    handler: function(request, reply) {
						...
						{
                            changeStoryCaption(db, poll_id ,user_id, old_caption, new_caption , function(returnVal, index) {
                                reply(returnVal);
                            });
                        } else { 
                            reply('{"status":-1,"msg":"Insufficient Information."}');
